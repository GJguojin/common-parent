...
public class Utility {
	...
	/**
	 * 自定义的分隔字符串函数 例如: 1,2,3 =>[1,2,3] 3个元素 ,2,3=>[,2,3] 3个元素 ,2,3,=>[,2,3,] 4个元素 ,,,=>[,,,] 4个元素 5.22算法修改，为提高速度不用正则表达式 两个间隔符,,返回""元素
	 * 
	 * @param split 分割字符 默认,
	 * @param src 输入字符串
	 * @return 分隔后的list
	 ...
	 * 
	 * @see test.koubei.util.UtilityTest#testParseStr()
	 * @param str
	 * @return
	 */
	...
	public static boolean equals( String s1, String s2 ) {
		if( Utility.isEmpty( s1 ) && Utility.isEmpty( s2 ) ) {
			return true;
		} else if( !Utility.isEmpty( s1 ) && !Utility.isEmpty( s2 ) ) {
			return s1.equals( s2 );
		}
		return false;
	}
	...
	/*
	 * String aaa="abcdefghijklmn"; String[] bbb=StringProcessor.midString(aaa, "b","l"); System.out.println("bbb[0]:"+bbb[0]);//cdefghijk
	 * System.out.println("bbb[1]:"+bbb[1]);//lmn ★这个方法是得到第二个参数和第三个参数之间的字符串,赋给元素0;然后把元素0代表的字符串之后的,赋给元素1
	 */
	...
	/**
	 * 泛型方法(通用)，把list转换成以“,”相隔的字符串 调用时注意类型初始化（申明类型） 如：List<Integer> intList = new ArrayList<Integer>(); 调用方法：Utility.listTtoString(intList);
	...
	public static Date convertStringToDate( String dateString, Date dateDif ) {
	...
	public static Date convertStringToDateTime( String dateString, Date dateDif ) {
		Date date = convertStringToDate( dateString );
		if( date != null ) {
			dateDif = date;
		}
	...
	public static String convertDateToStringDate( Date date, String stringDateDif ) {
		String stringDate = convertDateToStringDate( date );
		if( stringDate.trim().length() <= 0 ) {
			stringDate = stringDateDif;
		}
		return stringDate;
	}
	...
	public static String convertDateToStringDateTime( Date date, String stringDateDif ) {
		String stringDate = convertDateToStringDate( date );
		if( stringDate.trim().length() <= 0 ) {
			stringDate = stringDateDif;
		}
	...
	public static String convertCalendarToDateTimeMillString( Calendar calendar ) {
		String sDateString = "";
		...
		int nMillSecond = calendar.get( Calendar.MILLISECOND );

		sDateString += String.valueOf( nYear );

		sDateString += "-";
		sDateString = ( nMonth < 10 ) ? ( sDateString + "0" ) : sDateString;
		sDateString += String.valueOf( nMonth );

		sDateString += "-";
		sDateString = ( nDate < 10 ) ? ( sDateString + "0" ) : sDateString;
		sDateString += String.valueOf( nDate );

		sDateString += " ";
		sDateString = ( nHour < 10 ) ? ( sDateString + "0" ) : sDateString;
		sDateString += String.valueOf( nHour );

		sDateString += ":";
		sDateString = ( nMinute < 10 ) ? ( sDateString + "0" ) : sDateString;
		sDateString += String.valueOf( nMinute );

		sDateString += ":";
		sDateString = ( nSecond < 10 ) ? ( sDateString + "0" ) : sDateString;
		sDateString += String.valueOf( nSecond );

		sDateString += " ";
		sDateString += String.valueOf( nMillSecond );
	...
	// date string format: XXXX-XX-XX
	public static String convertDateStringToDateString( String dateString ) {
		String sDateString = "";
	...
	public static Calendar convertDateStringToCalendar( String dateString ) {
		...
		String sDateString = dateString.trim();
		int nIndex = sDateString.indexOf( "-" );
		if( nIndex != -1 ) {
			int nFromYear = Integer.parseInt( sDateString.substring( 0, nIndex ) );
			vCalendar.set( Calendar.YEAR, nFromYear );
			sDateString = sDateString.substring( nIndex + 1 );
			nIndex = sDateString.indexOf( "-" );
			if( nIndex != -1 ) {
				int nFromMonth = Integer.parseInt( sDateString.substring( 0, nIndex ) );
				vCalendar.set( Calendar.MONTH, nFromMonth - 1 );
				int nFromDate = Integer.parseInt( sDateString.substring( nIndex + 1 ) );
				vCalendar.set( Calendar.DATE, nFromDate );
			} else {
				return null;
			}
		} else {
	...
	// date string format: XXXX-XX-XX
	public static String convertCalendarToDateString6( Calendar calendar ) {
		String sDateString = "";
		...
		sDateString += String.valueOf( nYear );
		sDateString = sDateString.substring( 2 );
		sDateString = ( nMonth < 10 ) ? ( sDateString + "0" ) : sDateString;
		sDateString += String.valueOf( nMonth );
	...
	public static String createRadomNum( int num ) {
	...
	/**
	 * 用指定的模式和默认语言环境的日期格式符号构造时间字符串。注：此构造方法可能不支持所有语言环境。
	 * 
	 * @param date 要格式化的日期
	 * @param pattern 描述日期和时间格式的模式,默认为"yyyy-MM-dd"
	 * @return 给定日期的字符串形式.
	 */
	...
	public static String getPostfix( String path ) {
		if( path == null || ProjectCodeBook.EMPTY.equals( path.trim() ) ) {
			return ProjectCodeBook.EMPTY;
		}
		if( path.contains( ProjectCodeBook.POINT ) ) {
			return path.substring( path.lastIndexOf( ProjectCodeBook.POINT ) + 1, path.length() );
		}
		return ProjectCodeBook.EMPTY;
	}
	...
	private static String fillChartToLeft( String str, char fillChar, int len ) {
		if( str == null )
			return "";
		...
		if( strLength > len ) {
			...
			sbuffer.delete( 0, sbuffer.length() - len );
			return sbuffer.toString();
		}
		// <
		sbuffer = new StringBuffer();
		for( int i = 0; i < ( len - strLength ); i++ ) {
			sbuffer.append( fillChar );
		}
		sbuffer.append( str );
		return sbuffer.toString();
	}

	public static String paddingLeft( String str, int len ) {
		return paddingLeft( str, '0', len );
	}

	public static String paddingLeft( String str, char paddingCar, int len ) {

		return fillChartToLeft( str, paddingCar, len );
	}

	public static String fillChartToLeft( String str, int len ) {
		return fillChartToLeft( str, '0', len );
	}
	 ...
	 * @return
	 */
	public static String getCellPhoneValidateCode() {
		int number = getRandom(); // 生成六位数字的验证码
		String code = "000000" + number;
